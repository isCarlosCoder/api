...
let { welCard } = (() => {
  try {
    ...
    console.error(`Não foi possível importar "@delirius/welcard".`);
    return {};
  }
})();
...
const tryRequire = (moduleName) => {
  try {
    ...
    console.error(`Não foi possível importar "${moduleName}".`);
    return null;
  }
};
...
router.get('/canvas/welcome', async (req, res) => {
  ...
  if (!username || !groupname || !profile || !titulo || !isUrl(profile)) {
    const errorResponse = {
      status: false,
      message: 'Você deve fornecer os parâmetros necessários e válidos: título, nome de usuário, nome do grupo e perfil.',
...
router.get('/canvas/welcome2', async (req, res) => {
  ...
  if (!username || !groupname || !profile || !membercount || !description) {
    const errorResponse = {
      status: false,
      message: 'Você deve fornecer os parâmetros necessários: nome de usuário, nome do grupo, membercoun, descrição e perfil.',
...
router.get('/canvas/welcome5', async (req, res) => {
  ...
  if (!username || !groupname || !profile || !membercount || !background || !groupicon) {
    const errorResponse = {
      status: false,
      message: 'Você deve fornecer os parâmetros necessários: nome de usuário, nome do grupo, ícone do grupo, contagem de membros, perfil e histórico.',
...
router.get('/canvas/goodbye', async (req, res) => {
  ...
  if (!username || !groupname || !profile || !membercount || !background || !memberdiscriminator) {
    const errorResponse = {
      status: false,
      message: 'Você deve fornecer os parâmetros necessários: nome de usuário, nome do grupo, número de membros, perfil, discriminador de membros e histórico.',
...
router.get('/canvas/goodbye3', async (req, res) => {
  ...
  if (!username || !profile || !membercount || !background) {
    const errorResponse = {
      status: false,
      message: 'Você deve fornecer os parâmetros necessários: nome de usuário, número de membros, perfil e histórico.',
